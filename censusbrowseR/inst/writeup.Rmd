...
# If taking a class, will defend over the summer
# If not, will defend early fall
...
_Mention Dr. Hofmann's paper to improve the Statistical Atlas here_
...
_Look into what other things are available, different sources of data (NHGIS at U of Minn., different license). What issues there are with different sources of data_  
...
The available variable names here are already ordered by how many years they appear in, so we can see right off the bat that we have a serious lack of variable continuity. The identifying variables that appear each year ar `YEAR`, `STATE`, `TOTAL.POPULATION`, and `TYPE`. That means the total number of times they appear is 10, the number of census years we are looking at. After that, the next most common variable only appears in 4 of the 10 years we have chosen. That means we are up against some pretty significant hurdles to track any demographic group over time.  
