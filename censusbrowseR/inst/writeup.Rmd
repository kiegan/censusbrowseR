...
The majority of files for the decennial censuses were complete upon capture from the website. However, two years of census data were missing some column names and thus needed to be verified. For the 1890 and 1940 censuses, each column was compared to all columns from ICPSR 2896 and ICPSR 3 data files by both correlation and Euclidean distance, and thus some columns were able to be correctly identified.  

_Describe this process in more detail once I actually get this figured. How many columns were correctly matched? How many weren't?_  

_Currently I am struggling with the available data formats from ICPSR. I was able to compare the ICPSR 2896, but not many of the variables were actually able to be matched up, and I suspect this is because the ICPSR 2896 has some different values than the ICPSR 3. The ICPSR 3 is looking much more promising for verifying the column names, but... it doesn't give column names. I have the variable names in a codebook, but I am stuck between manually entering in all of the column names (there are 259 of them) and finding an overly complicated way to automate that processs..._  
...
```{r chunk-1790-map, fig.width = 4, fig.height = 2.33, echo = FALSE, message = FALSE, warning = FALSE, fig.pos = "H", fig.cap = "Total number of slaves per state in 1790, plotted on a continuous log scale. State boundaries for July 4, 1790 were gathered from `USAboundaries` package."}
...
```{r, fig.width = 8.33, fig.height = 5, echo = FALSE, message = FALSE, fig.pos = "H", fig.cap = "Number of Slaves, Number of Free Colored Persons, and Percentage of Free Colored Persons in 1820."}
...
p1_1820 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data = data_full_wmap_1820, 
               aes(x = long, y = lat, group = group, fill = log(TOTAL.SLAVES)), colour = "grey10") + 
  theme_bw() + labs(title = "Total Slaves (log scale)", x = "", y = "") + theme(legend.title = element_blank())
...
p2_1820 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1820, 
               aes(x = long, y = lat, group = group, fill = log(TOTAL.FREE.COLORED.PERSONS)), colour = "grey10") + 
  theme_bw() + labs(title = "Total Free Colored Persons (log scale)", x = "", y = "") + 
...
p3_1820 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1820, 
               aes(x = long, y = lat, group = group, fill = PERC.AFRICAN.AMERICANS.FREE), colour = "grey10") + 
  theme_bw() + labs(title = "Percent of Colored Persons Free", x = "", y = "") + 
  theme(legend.title = element_blank()) + scale_fill_gradient2(midpoint = .5)
...
```{r, fig.width = 8.33, fig.height = 5, echo = FALSE, message = FALSE, fig.pos = "H", fig.cap = "Number of Slaves, Number of Free Colored Persons, Percentage of Free Colored Persons, and Total African American Persons in 1850."}
...
p2_1850 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1850, 
               aes(x = long, y = lat, group = group, fill = log(TOTAL.FREE.COLORED.POPULATION)), colour = "grey10") + 
  theme_bw() + labs(title = "Total Free Colored Persons (log scale)", x = "", y = "") + 
  theme(legend.title = element_blank())
...
p4_1850 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1850, 
               aes(x = long, y = lat, group = group, fill = log(TOTAL.FREE.COLORED.POPULATION + TOTAL.SLAVES.ZERO)), colour = "grey10") + 
  theme_bw() + labs(title = "Total African American Persons (log scale)", x = "", y = "") + 
  theme(legend.title = element_blank()) 
...
#  theme_bw() + labs(title = "Total African American Persons (log scale)") + 
#  theme(legend.title = element_blank())
...
p4_1860 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1860, aes(x = long, y = lat, group = group, fill = log(TOTAL.FREE.COLORED.MALES + TOTAL.FREE.COLORED.FEMALES + AGGR..SLAVES.ZERO)), colour = "grey10") + 
  theme_bw() + labs(title = "1860", x = "", y = "") + 
  theme(legend.title = element_blank())
...
p4_1870 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group),
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1870, aes(x = long, y = lat, group = group, fill = log(COLORED.PERSONS)), colour = "grey10") + 
  theme_bw() + labs(title = "1870", x = "", y = "") + theme(legend.title = element_blank())
...
p4_1880 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group),
                        colour = "grey90", fill = "white") + 
  geom_polygon(data = data_full_wmap_1880, aes(x = long, y = lat, group = group, fill = log(COLORED.PERSONS)), colour = "grey10") + 
  theme_bw() + labs(title = "1880", x = "", y = "") + theme(legend.title = element_blank())
  
  
grid.arrange(p5_1850, p4_1860, p4_1870, p4_1880, ncol = 2, top = "Total Colored Persons (log scale)")
```
...
```{r, fig.width = 8.33, fig.height = 5, echo = FALSE, message = FALSE, fig.pos = "H", fig.cap = "Total Black Persons in 20th Century United States"}  
...
p1_1900 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data = data_full_wmap_1900, 
               aes(x = long, y = lat, group = group, fill = log(NEGRO.FEMALES + NEGRO.MALES)), colour = "grey10") + 
  labs(title = "1900", x = "", y = "") + theme(legend.title = element_blank())
...
p1_1910 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data = data_full_wmap_1910, 
               aes(x = long, y = lat, group = group, fill = log(TOTAL.NEGROES)), colour = "grey10") + 
  labs(title = "1910", x = "", y = "") + theme(legend.title = element_blank())
...
p1_1920 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data = data_full_wmap_1920, 
               aes(x = long, y = lat, group = group, fill = log(MALE.NEGROES + FEMALE.NEGROES)), colour = "grey10") + 
  labs(title = "1920", x = "", y = "") + theme(legend.title = element_blank())
...
p1_1930 <- ggplot() + geom_polygon(data = mcurr_df, aes(x = long, y = lat, group = group), 
  ...
  geom_polygon(data = data_full_wmap_1930, 
               aes(x = long, y = lat, group = group, fill = log(NEGRO.FEMALES + NEGRO.MALES)), colour = "grey10") + 
  labs(title = "1930", x = "", y = "") + theme(legend.title = element_blank())

grid.arrange(p1_1900, p1_1910, p1_1920, p1_1930, ncol = 2, top = "Total Black Persons (log scale)")
```


A period of growth and change can be seen at the turn of the 20th century as well, with a dramatic shift in the African American population out west to California and with the balance in the east and midwest shifting slightly further up north. Although the highest density still remains in the southern states, somewhat of a northern migration is visible.   
